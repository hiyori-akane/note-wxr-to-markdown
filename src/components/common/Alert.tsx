...
export function Alert({ type, children, onClose }: AlertProps) {
  const typeClasses = {
    ...
    warning: 'bg-yellow-50 text-yellow-800 border-yellow-300 dark:bg-yellow-900/30 dark:text-yellow-200 dark:border-yellow-700',
  ...
  return (
    <div className={`p-4 border rounded-lg ${typeClasses[type]} relative`} role="alert">
      {children}
      {onClose && (
        <button
          onClick={onClose}
